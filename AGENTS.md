...
- **`Palette::from_ui(ui)` Theme Token Architecture**: All UI widgets and components MUST obtain color tokens via `Palette::from_ui(ui)` rather than hardcoding theme-specific `Color32` values or branching manually on `ui.visuals().dark_mode`. This ensures 100% theme parity and automatic high-contrast palette resolution across light and dark modes.
- **Toggle Switch & Segmented Control Rules**: Use `theme::toggle_switch(ui, &mut bool)` (pill track with sliding thumb) for all binary boolean settings. Use `theme::segmented_container` with `ButtonVariant::Primary` (active) / `ButtonVariant::Ghost` (inactive) for mutually exclusive multi-choice option groups.
- **Compact Single-Monitor Topology Bar**: Single-monitor setups MUST collapse the topology canvas to a compact 36px status bar (`Display 1 (Primary) ● Connected`) to maximize vertical screen space for the wallpaper gallery grid. Full 140px 2D topology canvas is reserved for multi-monitor setups (>=2).
- **Card Hover Scope & Touch Target Sizes**: Interactive overlay controls on gallery cards (such as the trash icon button) MUST scope hover detection to `ui.min_rect()` (the card container) and enforce a minimum 28×28px touch target size (`ui.spacing_mut().interact_size`).
- **2-Column Responsive Settings Grid**: Settings panel MUST dynamically split settings cards into a 2-column layout (`ui.columns(2)`) when `available_width >= 720.0`, eliminating unused vertical space on wide windows.
- **egui Toast Overlay System**: `ToastManager` (`toast.rs`) renders notifications as an `egui::Area` anchored top-right with fixed width (340px) and default layout — **never** `top_down_justified` or `interactable(false)`, which cause frame-by-frame area jitter. Toast events from the IPC background thread are delivered via `std::sync::mpsc::channel` and drained once per frame via `drain_events()` before `show()`. Alpha fade is computed inline per frame from `Instant::now()` against `created_at` on each `Toast` — no `egui-toast` dependency.
